      ...
      class ProviderImporter
        ...
        def extract_providers(doc, use_encounters=false)
          ...
          providers = performers.map do |performer|
            ...
            org = entity.xpath("./cda:representedOrganization")
            org_addr = entity.xpath("./cda:representedOrganization/cda:addr")
            ...
            provider[:provider_email] =  extract_data(entity, "./cda:assignedPerson/cda:providerEmail")
            provider[:street]       = extract_data(org_addr, "./cda:street")
            provider[:city]         = extract_data(org_addr, "./cda:city")
            provider[:state]        = extract_data(org_addr, "./cda:state")
            provider[:org_email]    = extract_data(org, "./cda:orgEmail")
